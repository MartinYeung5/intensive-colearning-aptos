...
驗證者節點透過私密網路直接與其他驗證者節點溝通。全節點是最終交易歷史記錄的外部驗證和/或傳播資源。他們從同級接收交易，並且可以在本地重新執行它們（與驗證器執行交易的方式相同）。全節點將重新執行交易的結果儲存到本地儲存。在此過程中，他們可以挑戰驗證者的任何犯規行為，並在是否試圖重寫或修改區塊鏈歷史時提供證據。這有助於減輕驗證者腐敗和/或共謀。

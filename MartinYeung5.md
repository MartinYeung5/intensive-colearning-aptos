...
當交易提交到Aptos區塊鏈時，驗證節點運行分散式共識協議，執行交易，並將交易和執行結果儲存在區塊鏈上。驗證者節點決定將哪些交易添加到區塊鏈以及添加順序。

Aptos 區塊鏈使用拜占庭容錯（BFT）共識協議讓驗證者節點就最終交易的帳本及其執行結果達成一致。驗證器節點處理這些交易並將它們包含在區塊鏈資料庫的本地副本中。這意味著最新的驗證器節點始終在本地維護區塊鏈當前狀態的副本。

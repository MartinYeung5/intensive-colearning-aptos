...
#### 驗證節點的組成
* Mempool:
Mempool是每個節點內的其中一個元件，它保存已提交到區塊鏈但尚未達成一致或執行的事務的記憶體緩衝區。此緩衝區在驗證器節點和全節點之間複製。

全節點的 JSON-RPC 服務將交易傳送到驗證器節點的記憶體池。 Mempool 對交易執行各種檢查，以確保交易有效性並防止 DOS 攻擊。當新交易通過初始驗證並添加到記憶體池時，它會被分發到網路中其他驗證器節點的記憶體池。

當驗證者節點暫時成為共識協議中的領導者時，共識會從記憶體池中提取交易並提出一個新的交易區塊。該區塊被廣播給其他驗證者，並包含該區塊中所有交易的總排序。然後，每個驗證者執行該區塊並就是否接受新的區塊提案提交投票。
<!-- Content_END -->
